...
async function main() {
    const params = program
                    ...
                    //.requiredOption("-t, --thread <number>", "thread amount", "2")
    ...
    threadStatus.updateDifficulty(BigInt(difficulty))

    const auth = await spawn(new Worker("./auth"))
    auth.claim(endpoint, privateKey, wrapeggfarm, os.cpus().length - 1, overDifficulty)

    //claim(endpoint, privateKey, wrapeggfarm, sumthreads, overDifficulty)
