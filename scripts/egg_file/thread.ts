...
import { getMsgData, checkWallet, writefs, readDiff } from "./utils";
...
export async function mine(threadIndex:number, recipient: string) {
    while(true){
        ...
        let wallet_check = await checkWallet(account, threadStatus.getTargetByDiff(diff))
        if(wallet_check.valid){
            let msgData = await getMsgData(wallet_check.hash, account, recipient)
            writefs(msgData, threadIndex)
        }
    }
}
