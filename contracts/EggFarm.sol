...
contract EggFarm is ERC20, Ownable {
    ...
    uint256 public constant BASE_DIFFICULTY_TARGET = type(uint256).max; // bottom 1/1,000,000 of the 256-bit hash space
    ...
    constructor(uint256 initialSupply) ERC20("EggFarm", "EGG") Ownable(msg.sender) {
        if (initialSupply > 0) {
    ...
    function testEcrecover(bytes32 ethSignedMessageHash, bytes32 r, bytes32 s, uint8 v) public view returns(address) {
        return ecrecover(ethSignedMessageHash, v, r, s);
    }
